...
class MetaEpiModelTestCase(unittest.TestCase):
    def setUp(self):
        self.travel = pd.DataFrame(
            {"A": [0.99, 0.1], "B": [0.01, 0.9]}, index=["A", "B"]
        )
        self.population = pd.DataFrame(
            {"Population": [100_000, 10_000]}, index=["A", "B"]
        )
    ...
    def test_initialize_populations(self):
        ...
        self.assertEqual(
            self.SIR.compartments_["S"].sum(), self.population["Population"].sum()
        )
    ...
    def test_travel(self):
        ...
        self.assertEqual(
            new_compartments.sum().sum(), self.population["Population"].sum()
        )
